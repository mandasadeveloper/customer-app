...
const fieldDelete= async (id)=>{     //delete filed
if(window.confirm("Are you sure this file is deleted")){
axios.delete(`${UrlHttp}/delete/${id}`).then(
getData()
);}
}
                  ...
                  >
                    {(provider) => (
                     <Card.Section>
                        <p {...provider.draggableProps} ref={provider.innerRef}{...provider.dragHandleProps}>                                                                                                                                 
                        <div className="dragstyle">
                        <p><Badge><Icon source={DragHandleMinor} color="base" /></Badge></p><p style={{fontWeight:"600"}}>{user.label}</p><Button size="slim" onClick={(e)=>fieldDelete(e,user.id)}><Icon source={MobileCancelMajor}/></Button>
                        </div>           
                      </p>
                     </Card.Section>
                    )}
